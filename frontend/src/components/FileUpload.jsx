...
const FileUpload = () => {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get('http://16.171.16.76:3000/images');
      if (response.data) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const response = await axios.post("http://16.171.16.76:3000/upload", formData, {
  ...
  const handleDelete = async (name) => {
    try {
      await axios.delete("http://16.171.16.76:3000/delete", {
        data: { name },
      });
